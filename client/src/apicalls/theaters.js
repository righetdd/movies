...
export const GetAllTheatersByOwner = async (payload) => {
  try {
    const response = await axiosInstance.post("/api/theaters/get-all-theaters-by-owner", payload);
    return response.data;
  } catch (error) {
    return error.response;
  }
...
export const GetShowById = async (payload) => {
  try {
    const response = await axiosInstance.post("/api/theaters/get-show-by-id", payload);
    return response.data;
  } catch (error) {
    return error.response;
  }
